...
public class MagicalAllayshipClient implements ClientModInitializer {
	public static final Set<Integer> TRANSFORMED_PLAYERS = new HashSet<>();
	...
	public void onInitializeClient() {
		...
		ClientPlayNetworking.registerGlobalReceiver(SetTransformationPayload.TYPE, (payload, context) -> {
			if (payload.transformed()) {
				TRANSFORMED_PLAYERS.add(payload.playerId());
			} else {
				TRANSFORMED_PLAYERS.remove(payload.playerId());
			}
		});
		...
		LivingEntityRenderLayerRegistrationCallback.EVENT.register((entityType, entityRenderer, registrationHelper, context) -> {
			...
			registrationHelper.register(new AllaysuitLayer(playerRenderer, new AllaysuitModel(context.bakeLayer(AllaysuitModel.LAYER_LOCATION))));
		});
